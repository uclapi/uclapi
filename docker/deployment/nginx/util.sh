...
parse_domains() {
    sed -n -r -e 's&^\s*ssl_certificate_key\s*\/etc/letsencrypt/live/(.*)/privkey.pem;\s*(#.*)?$&\1&p' /usr/local/nginx/conf/conf.d/*.conf* | sort | uniq | xargs echo
}
...
auto_enable_configs() {
    for conf_file in /usr/local/nginx/conf/conf.d/*.conf*; do
...
template_user_configs() {
    SOURCE_DIR="${1-/usr/local/nginx/conf/conf.d/user.conf.d}"
    TARGET_DIR="${2-/usr/local/nginx/conf/conf.d/conf.d}"
