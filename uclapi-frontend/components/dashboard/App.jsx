...
export default class App extends React.Component {
  ...
  render() {
    ...
    if (this.DEBUGGING) {
      console.log("re-rendering app, name: " + app.name);
    }
    ...
    return (
      <PanelGroup accordion className="multi-wrapper">
        <Panel header={app.name} className={styles.panel}>
          <Container noPadding>
            <Row width="1-1">
              <Column width="1-1" className="title-holder">
                <Field
                  title="Title"
                  content={app.name}
                  onSave={(value) => {
                    actions.renameProject(index, value);
                  }}
                  isSmall
                />
                <Field
            ...
            <Container noPadding>
              <Column width="1-1" noPadding>
                <PanelGroup accordion className="narrow">
                  <Panel
                    header={"OAuth Settings"}
                    className={`narrow ${styles.panel} ${styles.subPanel}`}
                  >
                    <Container noPadding>
                      <Column width="1-1" className={styles.settingsSection}>
                        ...
                        <Field
                          ...
                          canCopy
                          onSave={(value) =>
                            actions.saveOAuthCallback(index, value)
                          }
                        />
                      </Column>
                    </Container>
                      ...
                      <Column width="1-1" className={styles.settingsSection}>
                        ...
                        {app.oauth.scopes.map((scope, scope_index) => (
                          <CheckBox
                            ...
                            onClick={(value) =>
                              actions.setScope(index, scope_index, value)
                            }
                          />
                        ))}
                      </Column>
                  ...
                  >
                    <Container noPadding>
                      <Column width="1-1">
                        {["requests", "remaining_quota"].map(
                          (analytic, analytic_index) => (
                            <AnalyticInfo
                              key={analytic_index}
                              analytic={analytic}
            ...
            <Container noPadding>
              <Column width="1-1" className="bottom-app-column" keepInline>
                <Column
                  width="165px"
                  alignItems="column"
