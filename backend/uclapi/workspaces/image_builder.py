...
class ImageBuilder:
    ...
    def get_live_map(self):
        map_data = self._api.get_survey_image_map_data(self._survey_id, self._map_id)
        nsmap = {
        ...
        scale = "scale({}, {})".format(self._image_scale, self._image_scale)
        ...
        map_data = self._redis.hgetall("occupeye:surveys:{}:maps:{}".format(self._survey_id, self._map_id))
        (b64_data, content_type) = self._api.get_image(map_data["image_id"])
        base_map.attrib["{http://www.w3.org/1999/xlink}href"] = "data:{};base64,{}".format(content_type, b64_data)
        ...
        for sensor_id, sensor_data in the_map["sensors"].items():
            ...
            node.attrib["transform"] = "translate({},{})".format(sensor_data["x_pos"], sensor_data["y_pos"])
