-- Special case of generate_timetable.sql that will get all
-- timetable slots for a student's modules.
-- This will not generate a timetable; doing that is a work in progress.
-- This script is also limited to bucket A.

CREATE OR REPLACE FUNCTION get_student_module_slots_a(
    TEXT, -- UPI
    TEXT  -- Set ID
)
RETURNS TABLE (
...
-- Return the timetable we just built back to the calling application
    RETURN QUERY SELECT * FROM tt_tmp_events_slot_id;
